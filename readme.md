...
<b>Left: Dataset image overlayed with label. Right: Same dataset image overlayed with prediction.</b>
...
So I started anew with creating a dataset for the steering predictions. I captured frames of the vehicle at the center of the path, looking directly forward down the path. In the csv I used for the labels I assigned steer values of numbers very close to 0.0. I then turned the vehicle about 80 degrees to the left while still placed in the center of the path. I then captured screenshots while turning to the right (1.0 steering value), and assigned lables evenly spaced from 1.0 to 0, thus telling the vehicle to turn sharply to the right whenever it found itself in the center of the path but turned toward the lane markings, and do minor corrections when nearly looking forward but looking left of center. I then repeated this with the vehicle turned 80 degrees to the right, and assigned it labels evenly arranged from -1 to 0. For cases where the vehicle found itself near a lane marking, either to the left or right side of the path, and looking straight down the path, I captured screenshots as I gently maneuvered back toward center and aligned it to look directly forward down the path. I assigned these labels ranging evenly from 0.5 to 0 and -0.5 to 0 respectively, telling the vehicle to move gradually from the edge of the path to the center and keep it aligned to move down the path.

With this more carefully created dataset I then trained the cnn using the steerPred_steerPredict notebook. With this data I was able to get much better losses, eventually ending up around .005, a 100 times improvement. For some reason that I don't really understand, I wasn't able to get my training loss to be less than my validation loss, as I was advised to in the fast.ai course, but the results looked good and my predictor was giving me steering values really close to my labels. Later, I realized using resnet34 for transfer learning was probably overkill for this problem. I wasn't trying to really recognize anything but line angles and a simpler architecture would be more efficient.

Using the same setup for Unreal as described above, I finally got my vehicle to stay between the red lines. I implemented control overrides that allow the user to nudge the vehicle either left of right, to test it's abilitiy to correct its steering and stay on course.
...
It's been a great learning project for me. I'm not an exerienced coder, so it's been challenging to have to figure out a lot of new Python functionality, but I'm still really excited about combining these new skills and my past experience using 3D game engines to create datasets and test machine learning ideas.
